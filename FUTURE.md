...
- **Gmail: explore middleware for parameter enforcement** — FastMCP 3.x supports `on_call_tool` middleware that can intercept and transform arguments before tool execution. Could be useful for future parameter validation beyond the current error-based approach.
...
- **Gmail: attachment elicitation per-file** — Currently elicitation is all-or-nothing for downloadable attachments. Allow selecting which files to download individually.
